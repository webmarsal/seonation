@section('site_title', formatTitle([__('Reverse IP lookup'), __('Tool'), config('settings.title')]))
...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('tools.reverse_ip_lookup') }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row mx-n2">
                ...
                <div class="col-auto px-2">
                    <a href="{{ route('tools.reverse_ip_lookup') }}" class="btn btn-outline-secondary ml-auto">{{ __('Reset') }}</a>
                </div>
            </div>
        </form>
    </div>
