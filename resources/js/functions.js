...
if (document.querySelector('#form-tools-search')) {
    document.querySelector('#i-search').addEventListener('keyup', function () {
        ...
        if (document.querySelector('#i-search').value.length > 0) {
            ...
            document.querySelectorAll('#tools [data-tool-category]').forEach(function (item) { item.classList.add('d-none') });
        } else {
            ...
            // Show the category labels
            document.querySelectorAll('#tools [data-tool-category]').forEach(function (item) { item.classList.remove('d-none') });
        }

        document.querySelectorAll('#tools [data-tool-title]').forEach(function (item) {
            // Initially hide the tool
            item.classList.add('d-none');

            // If the tool title contains the search query
            if (item.dataset.toolTitle.toLowerCase().includes(document.querySelector('#i-search').value.toLowerCase())) {
                // Show the tool
                item.classList.remove('d-none');

                document.querySelector('#tools [data-tool-category="' + item.dataset.toolParent + '"]').classList.remove('d-none');
            }
        });
    });
    ...
    document.querySelector('#b-clear').addEventListener('click', function () {
        ...
        // Show the category labels
        document.querySelectorAll('#tools [data-tool-category]').forEach(function (item) { item.classList.remove('d-none') });
        document.querySelectorAll('#tools [data-tool-title]').forEach(function (item) { item.classList.remove('d-none') });
    });
}
